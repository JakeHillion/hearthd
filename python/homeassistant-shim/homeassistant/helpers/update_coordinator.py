...
class DataUpdateCoordinator(Generic[T]):
    ...
    async def _async_notify_listeners(self) -> None:
        ...
        # After notifying listeners, send state updates for registered entities
        await self._async_send_entity_states()

    async def _async_send_entity_states(self) -> None:
        """Send state updates for all entities using this coordinator."""
        for listener_ref in self._listeners:
            ...
            if send_fn is not None:
                try:
                    ...
                    _LOGGER.debug("Failed to send entity state to Rust", exc_info=True)
