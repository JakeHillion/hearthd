...
async fn send_entity_command(
    ...
    tracing::debug!(
        "Handling POST /v1/entities/{}/command: {:?}",
        entity_id,
        request.command
    );

    // For now, only support light commands
    if request.command == "light" {
        let on = request.on.unwrap_or(false);
        ...
        (
            ...
            Json(EntityCommandResponse {
                success: false,
                message: format!("Unknown command type: {}", request.command),
            }),
        )
            .into_response()
    }
