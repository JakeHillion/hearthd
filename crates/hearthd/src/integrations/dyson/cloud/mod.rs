...
impl DysonAccount {
    ...
    pub async fn request_email_otp(&mut self, email: &str, region: &str) -> Result<()> {
        // NATIVE FLOW (mirrors libdyson.account.login_email_otp): first check
        // the account exists and is registered in this region.
        let status_path = "/v3/userregistration/email/userstatus";
        let status: serde_json::Value = self
            .request(
                Method::POST,
                status_path,
                Some(&[("country", region)].into_iter().collect::<HashMap<_, _>>()),
                Some(&serde_json::json!({ "email": email })),
                false,
            )
            .await
            .context("failed to check account status")?;
        let account_status = status["accountStatus"]
            .as_str()
            .context("account status response missing accountStatus")?;
        if account_status != "ACTIVE" {
            anyhow::bail!("Dyson account is not active (status: {account_status})");
        }
