...
pub fn group_node(
    ...
    let mut input_list = streams
        .values()
        .filter_map(|s| {
            // A stream with no index is one `SelectInput` cannot reach, and
            // giving every such stream index 0 would list them all under the
            // one index and shadow the stream that really holds it.
            let index = stream_indices.get(&s.id).copied()?;
            Some(InputInfo {
                index,
    ...
    // Left absent rather than defaulted to 0, which names a real stream: a
    // group pointing at one the server did not describe would otherwise report
    // the first stream as the selected one.
    let current_input = stream_indices.get(&group.stream_id).copied();
