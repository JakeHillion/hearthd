...
impl PrettyPrint for LirInstr {
    fn pretty_print(&self, indent: usize, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // Labels sit one indent level out from instructions for visual
        // grouping; everything else renders at the given indent.
        let indent = match self {
            LirInstr::Label(_) => indent.saturating_sub(1),
            _ => indent,
        };
        write_indent(indent, f)?;
        write_instr(self, f)?;
...
fn write_instr(instr: &LirInstr, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match instr {
        LirInstr::Label(lbl) => write!(f, "{}:", lbl),
