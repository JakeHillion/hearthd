...
//!
//! Firings run side by side. Each one gets its own [`Program::instance`] — its own
...
use crate::automations::parser::ast::AutomationKind;
use crate::automations::schema::DeploymentSchema;
use crate::automations::vm::Program;
...
impl CompiledAutomation {
    ...
    pub fn new(kind: AutomationKind, filter: Bytecode, body: Bytecode) -> Result<Self, VmError> {
        ...
        Ok(Self {
            kind,
            filter: Mutex::new(Arc::new(Program::new(filter)?).instance()),
            filter_bindings,
            body: Arc::new(Program::new(body)?),
            body_bindings,
...
impl Suspension for Instance {
    /// `sleep` is left to the trait's default, which waits the duration out:
    /// nothing may cut one short, so a firing being superseded has no say in
    /// it.
    async fn sleep_unique(&self, duration: Duration) -> bool {
        ...
        tokio::select! {
            ...
            // `wait_for` tests the current value before waiting at all, so an
            // instance superseded while it sat in an earlier `sleep` fails
            // here without serving any of this wait. That is the one case the
            // two rules above do not dictate between them, and this is the
            // reading they compose to: the `sleep` ran because nothing may cut
            // one short, and the `sleep_unique` fails because by the time it
            // is reached a newer instance already exists.
            _ = async {
                if newest.wait_for(|newest| *newest > generation).await.is_err() {
                    // The automation is gone, so no firing can follow this
                    // one; leave the wait to decide.
...
fn state_to_value(state: &State, schema: &DeploymentSchema) -> Value {
    ...
    for (domain, slugs) in &schema.domains {
        ...
        domains.insert(domain.as_str().to_string(), Value::Struct(nodes));
    }
...
/// Project one node.
///
/// A [`Value::Node`] rather than a snapshot of its fields, because that is
/// what naming an entity already produces: `state.light.living_room_lamp`
/// relocates to a node handle, so a node reached any other way has to be the
/// same thing or the two would compare unequal.
...
fn dispatch_actions(sink: &dyn ActionSink, result: Value) {
    ...
    for item in items {
        ...
        match args.as_slice() {
            [Value::Node(node_id)] => sink.invoke_command(*node_id, ACTION_ENDPOINT, command),
