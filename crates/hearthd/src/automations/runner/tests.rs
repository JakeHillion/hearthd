...
/// Compile one observer against `schema`, asserting it type-checks cleanly.
fn compile_observer(source: &str, schema: Arc<DeploymentSchema>) -> CompiledAutomation {
    ...
    let checked = crate::automations::check_program_with_schema(&lowered, schema);
    assert!(
    ...
    let auto = match crate::automations::lower_bytecode_program(&lir) {
        crate::automations::repr::BytecodeProgram::Automation(auto) => auto,
        crate::automations::repr::BytecodeProgram::Template { .. } => {
            panic!("expected an Automation, got a Template")
        }
    };
...
async fn test_runner_re_triggering_restarts_the_window() {
    ...
    let runner = Runner::new(
        ...
        schema.clone(),
        vec![
            compile_observer(MOTION_ON, schema.clone()),
...
fn test_runner_action_event_carries_its_node() {
    insta::assert_snapshot!(
        ...
        @r#"Event::LightOn({entity_id: "light.living_room_lamp", id: node#1, integration: "test", name: ()})"#
    );
}
...
fn test_runner_state_projects_the_schema_shape() {
    ...
    insta::assert_snapshot!(
        ...
        @r#"{binary_sensor: {kitchen_motion: {entity_id: "binary_sensor.kitchen_motion", id: node#2, integration: "test", name: ()}}, light: {living_room_lamp: {entity_id: "light.living_room_lamp", id: node#1, integration: "test", name: ()}}}"#
    );
}
...
fn test_runner_state_omits_a_departed_node() {
    ...
    insta::assert_snapshot!(
        ...
        @r#"{binary_sensor: {kitchen_motion: {entity_id: "binary_sensor.kitchen_motion", id: node#2, integration: "test", name: ()}}, light: {}}"#
    );
}
...
async fn test_runner_the_shipped_examples_drive_a_light() {
    ...
    let runner = Runner::new(
        ...
        schema.clone(),
        vec![
            compile_observer(EXAMPLE_MOTION_ON, schema.clone()),
