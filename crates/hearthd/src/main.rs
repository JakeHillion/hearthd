...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // checked once those devices are known. Integrations discover
    ...
    // the schema and an automation naming it fails to compile.
    ...
    for automation in &sources {
        match compile_automation(automation, schema.clone()) {
...
/// check against the deployment schema, then lower to HIR, LIR and bytecode —
/// and build the VMs the runner executes.
///
/// The error is a rendered diagnostic rather than a value, because the only
/// thing that happens to it is being logged; a parse or type failure names
/// the span it came from the same way the tests do.
fn compile_automation(
    ...
    let auto = match hearthd::automations::lower_bytecode_program(&lir) {
        hearthd::automations::repr::BytecodeProgram::Automation(auto) => auto,
