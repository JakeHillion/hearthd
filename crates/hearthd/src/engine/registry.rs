...
//!
//! Moving a node to a different name is a third verb,
//! [`RegisteredNode::rename`], for integrations whose entity id is derived
//! from a device name the operator can change. It releases the old name and
//! claims the new one under the same [`CollisionPolicy`], so a name never
//! leaves the keyspace by being quietly overwritten.
...
impl RegisteredNode {
    ...
    /// Move the node to a new name, releasing the one it holds.
    ///
    /// For integrations whose entity id is derived from a device name the
    /// operator can change — a Snapcast group, say — the name a node answers
    /// to is not settled at registration. A rename is still a claim on the
    /// shared keyspace though, so it goes through the registry and obeys the
    /// same [`CollisionPolicy`]; the name held afterwards is
    /// [`Self::entity_id`], which may not be the one asked for.
    ///
    /// A separate verb from [`Self::announce`] for the same reason `register`
    /// is: announcing re-publishes the node under the name it already holds
    /// and cannot move it, so the caller has to say which it means.
    ///
    /// Under [`CollisionPolicy::Reject`] a refused rename leaves the node
    /// holding the name it already had, so it costs the new name rather than
    /// the node.
    pub fn rename(&mut self, requested: &str) -> Result<(), RegisterError> {
        if requested == self.entity_id {
            return Ok(());
        }
        ...
        let assigned = {
            ...
            // Resolved as though this node were not registered. Its own name
            // must not block its own rename, and a node holding `x_2` only
            // because `x` was taken has to be able to re-take `x_2` when it
            // asks for `x` again — otherwise every refresh that re-derived the
            // same requested name would climb it another suffix.
            let held = if inner.by_entity_id.get(&self.entity_id) == Some(&self.node_id) {
                inner.by_entity_id.remove(&self.entity_id)
            } else {
                None
            };

            match inner.resolve_name(requested, self.shared.policy) {
                Ok(assigned) => {
                    inner.by_entity_id.insert(assigned.clone(), self.node_id);
                    assigned
                }
                Err(e) => {
                    // Put back what was given up in order to ask: a refused
                    // rename must not also drop the reservation the node
                    // arrived with.
                    if let Some(node_id) = held {
                        inner.by_entity_id.insert(self.entity_id.clone(), node_id);
                    }
                    return Err(e);
                }
            }
        ...
        if assigned != requested {
            warn!(
                "{} asked to rename node {} to '{}', which is taken; renamed to '{}' instead",
                self.integration, self.node_id, requested, assigned
            );
        }
        self.entity_id = assigned;
        Ok(())
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn renaming_frees_the_old_name_and_claims_the_new_one() {
        let (registry, _rx) = registry(CollisionPolicy::Reject);
        let mqtt = registry.for_integration("mqtt");

        let mut handle = mqtt.register(node("media_player.a")).await.unwrap();
        handle.rename("media_player.b").unwrap();

        assert_eq!(handle.entity_id(), "media_player.b");
        // The old name is a reservation like any other, and holding it after
        // the node stopped answering to it would deny it to whoever wants it.
        let reused = mqtt.register(node("media_player.a")).await.unwrap();
        assert_ne!(reused.node_id(), handle.node_id());
    }

    #[tokio::test]
    async fn renaming_to_the_name_already_held_is_a_no_op() {
        let (registry, _rx) = registry(CollisionPolicy::Reject);
        let mqtt = registry.for_integration("mqtt");

        let mut handle = mqtt.register(node("media_player.a")).await.unwrap();
        handle
            .rename("media_player.a")
            .expect("a node must not collide with itself");

        assert_eq!(handle.entity_id(), "media_player.a");
    }

    /// A node that was suffixed at registration re-derives its *requested*
    /// name on every refresh. Asking again must settle on the suffix it
    /// already holds rather than climbing to the next one each time.
    #[tokio::test]
    async fn renaming_to_a_still_taken_name_keeps_the_suffix_already_held() {
        let (registry, _rx) = registry(CollisionPolicy::Suffix);
        ...
        for _ in 0..3 {
            second.rename("weather.home").unwrap();
            assert_eq!(second.entity_id(), "weather.home_2");
        }
    }

    #[tokio::test]
    async fn a_refused_rename_leaves_the_node_holding_its_old_name() {
        let (registry, _rx) = registry(CollisionPolicy::Reject);
        ...
        let _holder = registry
            ...
            .await
            .unwrap();
        let mut mine = registry
            .for_integration("mqtt")
            .register(node("weather.away"))
            .await
            .unwrap();

        let err = mine
            .rename("weather.home")
            .expect_err("the name is taken by another integration");
        assert_eq!(err.claimed_by, "metno");

        // The reservation it arrived with survives the failed attempt, so the
        // node stays addressable rather than falling out of the keyspace.
        assert_eq!(mine.entity_id(), "weather.away");
        assert_eq!(
            registry.shared.lock().by_entity_id.get("weather.away"),
            Some(&mine.node_id())
        );
    }

    #[tokio::test]
    async fn an_announcement_after_a_rename_carries_the_new_name() {
        let (registry, mut rx) = registry(CollisionPolicy::Reject);
        let mqtt = registry.for_integration("mqtt");
        ...
        handle.announce(node("media_player.ignored")).send().await;

        match rx.recv().await.unwrap() {
            FromIntegrationMessage::NodeAdded { node_id, node } => {
                assert_eq!(node_id, handle.node_id());
                assert_eq!(node.entity_id, "media_player.b");
            }
            other => panic!("expected NodeAdded, got {other:?}"),
        }
    }
