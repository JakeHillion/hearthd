...
pub fn expand_mergeable_config(input: DeriveInput, is_root: bool) -> Result<TokenStream> {
    ...
    let merge_impl = if is_root {
        generate_root_merge_impl(name, &field_infos)?
    } else {
...
fn generate_root_merge_impl(config_name: &Ident, fields: &[FieldInfo]) -> Result<TokenStream> {
    let partial_name = format_ident!("Partial{}", config_name);
    ...
    let merge_logic: Vec<_> = fields
        .iter()
        .map(|f| generate_field_merge(f, true))
...
fn generate_field_merge(field: &FieldInfo, _is_root: bool) -> Result<TokenStream> {
    let name = &field.name;
    let name_str = name.to_string();
